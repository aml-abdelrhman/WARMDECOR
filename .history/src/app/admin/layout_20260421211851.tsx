...
const isAuthenticated = useAuthStore((s) => !!(s.user && s.token));
  ...
  // نصيحة: يفضل نقل البريد الإلكتروني إلى NEXT_PUBLIC_ADMIN_EMAIL في ملف .env
  const isAdmin = userRole === "admin" || userEmail?.toLowerCase().trim() === process.env.NEXT_PUBLIC_ADMIN_EMAIL;
